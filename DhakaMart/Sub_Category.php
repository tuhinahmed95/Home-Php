...
<div class="flex gap-3 items-center justify-center my-5">
    ...
    <h1 class="text-center font-bold text-2xl">Sub-Category Table</h1>
    <div class="border-t flex-grow "></div>
</div>
<!-- Add Product -->
<div class="flex justify-end my-4">
    <button class="btn btn-md btn-secondary view-details " onclick="my_modal_1.showModal()">
        <i class="fa-regular fa-square-plus"></i> Add Sub-Category
    </button>
</div>
<!-- Products Table -->
<div class="">
    <table class="custom-table">
        ...
        <tbody>
            ...
            $query = "SELECT sub_category.id, sub_category.name, brand.name bname, category.name mname from sub_category, brand, category where sub_category.cat_id = category.id and sub_category.brand_id= brand.id";
            $query_run = mysqli_query($conn,  $query);
            if (mysqli_num_rows($query_run) > 0) {
                foreach ($query_run as $row) {
            ?>
